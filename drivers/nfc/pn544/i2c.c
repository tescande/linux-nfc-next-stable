...
static int pn544_hci_i2c_fw_upload(void *phy_id, const char *firmware_name)
{
	struct pn544_i2c_phy *phy = phy_id;

	pr_info(DRIVER_DESC ": Starting Firmware Upload (%s)\n",
		firmware_name);
...
static void pn544_hci_i2c_fw_work_complete(struct pn544_i2c_phy *phy,
					   int result)
{
	pr_info(DRIVER_DESC ": Firmware Upload Complete, result=%d\n", result);
	...
	nfc_fw_upload_done(phy->hdev->ndev, phy->firmware_name, (u32) -result);
}
			       ...
			       const struct i2c_device_id *id)
{
	...
	r = pn544_hci_probe(phy, &i2c_phy_ops, LLC_SHDLC_NAME,
			    ...
			    PN544_HCI_I2C_LLC_MAX_PAYLOAD,
			    pn544_hci_i2c_fw_upload, &phy->hdev);
	if (r < 0)
		goto err_hci;
